...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 bg-white right-0 z-50 transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-sm shadow-md py-3" : "bg-transparent py-5"}`}
    >
          ...
          <ul className="flex items-center gap-2">
            {navLinks.map((l) => (
              <li key={l.href}>
                <Link
                  to={l.href}
                  className="px-4 py-2 text-[15px] font-bold text-secondary/70 hover:text-accent transition-colors"
                >
                  {l.label}
                </Link>
              </li>
